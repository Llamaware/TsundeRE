...
@PluginInfo(
	...
	shortDescription = "TsundRE Client - Discord Integration",
	description = "TsundeRE Client - Discord Integration"
)
//@formatter:on
    ...
    private void updatePresenceInfo() {
		...
		if (repository != null) {
			if (repository.isConnected()) {
				...
				if (ApiClient.getUsers().length > 1) {
					List<String> userList = new ArrayList<>();
					for (String user : ApiClient.getUsers()) {
					    if (!user.equals(ApiClient.getClientName())) {
					        userList.add(user);
					    }
					}
					sb.append("On a server with ");
					sb.append(String.join(", ", userList));
				}
				else {
					sb.append("Connected to Ghidra server");
				}
				desc = sb.toString();
			}
			else {
